...
from utils_for_analysis import num_muts_column_name
...
def main():
    ...
    for n_mut in range(1, 6):
        indices_dict = []

        nmut_column_name = num_muts_column_name[dataset]

        total_indices = np.arange(len(df))

        indices_of_nmut = np.where(df[nmut_column_name] == n_mut)[0]
        test_indices = np.setdiff1d(total_indices, indices_of_nmut)

        size = len(indices_of_nmut)

        

        indices_dict.append({
            "size": size,
            "indices_of_nmut": indices_of_nmut,
        ...
        for i in range(args.iters):
            train_indices = np.random.choice(total_indices, size, replace=False)
            test_indices = np.setdiff1d(total_indices, train_indices)
        ...
        for ind in indices_dict:
            ...
            result_dict["cor_ohe"] = cor_ohe.correlation
