...
def parse_hidden_layers(hl_list):
    parsed = []
    for hl in hl_list:
        if isinstance(hl, str):
            hl_tuple = tuple(int(x) for x in hl.split(','))
        elif isinstance(hl, (tuple, list, np.ndarray)):
            hl_tuple = tuple(int(x) for x in hl)
        else:
            hl_tuple = (int(hl),)
        parsed.append(hl_tuple)
    return parsed
...
def main():
    ...
    # Accepts space-separated values, each value can be a comma-separated string for multilayer: "100" or "100,50"
    ...
    parser.add_argument('--alpha', type=float, nargs='+', default=[0.5], help="List of alpha values (ignored if not using grid search)")
    parser.add_argument('--op', type=str, nargs='+', default=['adam'], choices=['adam', 'lbfgs'],
                        help='Optimizers (ignored if not using grid search)')
    parser.add_argument('--niters', type=int, nargs='+', default=[50],
                        help='Number of #iterations (epochs) for training the classifier')
    parser.add_argument('--lr', type=float, nargs='+', default=[1e-3], help="List of learning_rates (ignored if not using grid search)")
    ...
    # Parse --hl argument into list of tuples of ints (keep full logic, but only use the first element below)
    hidden_layers_list = parse_hidden_layers(args.hl)
    ...
    # Use only the first element/config for each parameter
    hidden_layers = hidden_layers_list[0]
    alpha = args.alpha[0]
    solver = args.op[0]
    niters = args.niters[0]
    lr = args.lr[0]
    ...
    for n_train in range(args.n_start, args.n_end):
        ...
        print(f"\nParams: hidden_layers={hidden_layers}, alpha={alpha}, solver={solver}, niters={niters}, lr={lr}")
        ...
        if is_regression:
            mlp_ohe = MLPRegressor(hidden_layer_sizes=hidden_layers, **mlp_base_parameters)
            mlp_ohe.fit(X_train.numpy(), y_train.astype(float))
        else:
            mlp_ohe = MLPClassifier(hidden_layer_sizes=hidden_layers, **mlp_base_parameters)
        ...
        for i in n_test_data:
            ...
            # Record params in the evaluation
