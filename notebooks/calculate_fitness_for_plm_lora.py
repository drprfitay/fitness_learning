...
LORA_MODEL_KEY = None       # e.g. "esm_35m"; keep None when not using LoRA
LORA_WEIGHTS_PATH = None    # e.g. "./notebooks/data/pard3/lora_esm35m_msa"
...
def lora_kwargs_for_model(model_key):
    if LORA_WEIGHTS_PATH is None:
        return {}
    if LORA_MODEL_KEY is not None and model_key != LORA_MODEL_KEY:
        return {}
    if LORA_MODEL_KEY is None and len(tokenized_sequences_path_dict) > 1:
        raise ValueError("Set LORA_MODEL_KEY when scoring multiple PLMs with LoRA")
...
def build_scoring_model(model_key, dataset_to_use, wt_seq, lora_kwargs=None):
