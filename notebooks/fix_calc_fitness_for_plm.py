...
ADDITIVE_SCORE_BATCH_SIZE = 20000
...
def additive_scores_from_log_probs(
    tokenized_sequence,
    wt_tokens,
    working_positions,
    log_probs,
    ...
    log_probs = torch.as_tensor(log_probs).float().cpu()

    working_log_probs = log_probs[working_positions, :]
    wt_working_tokens = wt_tokens[working_positions]
    wt_log_probs = working_log_probs[
        torch.arange(len(working_positions)),
        wt_working_tokens,
    ]

    fitness = np.zeros(tokenized_sequence.shape[0], dtype=np.float32)

    for start in range(0, tokenized_sequence.shape[0], batch_size):
        end = min(start + batch_size, tokenized_sequence.shape[0])
        variant_tokens = tokenized_sequence[
            start:end,
            :,
        ][:, working_positions]

        variant_log_probs = working_log_probs.unsqueeze(0).expand(
            end - start,
            -1,
            -1,
        ).gather(
            2,
            variant_tokens.unsqueeze(2),
        ).squeeze(2)
...
for dataset_to_use in datasets.keys():
    ...
    for k in tokenized_sequences_path_dict.keys():
        ...
        fitness = additive_scores_from_log_probs(
            tokenized_sequence,
            wt_tokens,
            working_positions,
            log_probs,
            batch_size=ADDITIVE_SCORE_BATCH_SIZE,
        )
        ...
        print("[INFO] Fitness range: %.6f to %.6f" % (fitness.min(), fitness.max()))
